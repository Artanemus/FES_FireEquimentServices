object Suburb: TSuburb
  Left = 0
  Top = 0
  BorderStyle = bsDialog
  Caption = 'Pick Suburb'
  ClientHeight = 754
  ClientWidth = 646
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -16
  Font.Name = 'Segoe UI'
  Font.Style = []
  Position = poOwnerFormCenter
  TextHeight = 21
  object Panel2: TPanel
    Left = 0
    Top = 0
    Width = 646
    Height = 49
    Align = alTop
    BevelEdges = [beBottom]
    BevelKind = bkFlat
    BevelOuter = bvNone
    TabOrder = 0
    ExplicitWidth = 633
    object Label1: TLabel
      Left = 24
      Top = 14
      Width = 124
      Height = 21
      Caption = 'Search for suburb'
    end
    object SpeedButton3: TSpeedButton
      Left = 339
      Top = 8
      Width = 32
      Height = 32
      ImageIndex = 0
      ImageName = 'backspace'
      Images = VirtualImageList1
      Flat = True
    end
    object Edit1: TEdit
      Left = 154
      Top = 10
      Width = 179
      Height = 29
      TabOrder = 0
      Text = 'Edit1'
    end
  end
  object DBGrid1: TDBGrid
    AlignWithMargins = True
    Left = 10
    Top = 59
    Width = 626
    Height = 644
    Margins.Left = 10
    Margins.Top = 10
    Margins.Right = 10
    Margins.Bottom = 10
    Align = alClient
    BorderStyle = bsNone
    DataSource = dsSuburb
    Options = [dgTitles, dgIndicator, dgColumnResize, dgColLines, dgRowLines, dgTabs, dgRowSelect, dgConfirmDelete, dgCancelOnExit, dgTitleClick, dgTitleHotTrack]
    TabOrder = 1
    TitleFont.Charset = DEFAULT_CHARSET
    TitleFont.Color = clWindowText
    TitleFont.Height = -16
    TitleFont.Name = 'Segoe UI'
    TitleFont.Style = []
    Columns = <
      item
        Expanded = False
        FieldName = 'PostcodeID'
        Visible = False
      end
      item
        Expanded = False
        FieldName = 'Postcode'
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'Suburb'
        Visible = True
      end
      item
        Expanded = False
        FieldName = 'Zone'
        Visible = True
      end>
  end
  object Panel1: TPanel
    Left = 0
    Top = 713
    Width = 646
    Height = 41
    Align = alBottom
    BevelEdges = [beTop]
    BevelKind = bkFlat
    BevelOuter = bvNone
    TabOrder = 2
    ExplicitTop = 143
    ExplicitWidth = 215
    object btnOk: TButton
      Left = 285
      Top = 7
      Width = 75
      Height = 25
      Caption = 'OK'
      TabOrder = 0
    end
  end
  object qrySuburb: TFDQuery
    ActiveStoredUsage = [auDesignTime]
    Active = True
    Connection = FES.fesConnection
    SQL.Strings = (
      
        'SELECT dbo.Postcode.PostcodeID, dbo.Postcode.Postcode, dbo.Postc' +
        'ode.Suburb, dbo.Postcode.Zone'
      'FROM dbo.Postcode'
      'ORDER BY dbo.Postcode.Suburb;')
    Left = 376
    Top = 256
    object qrySuburbPostcodeID: TFDAutoIncField
      FieldName = 'PostcodeID'
      Origin = 'PostcodeID'
      ProviderFlags = [pfInWhere, pfInKey]
      ReadOnly = True
    end
    object qrySuburbPostcode: TIntegerField
      Alignment = taCenter
      DisplayWidth = 8
      FieldName = 'Postcode'
      Origin = 'Postcode'
    end
    object qrySuburbSuburb: TWideStringField
      DisplayWidth = 26
      FieldName = 'Suburb'
      Origin = 'Suburb'
      Size = 50
    end
    object qrySuburbZone: TWideStringField
      DisplayWidth = 30
      FieldName = 'Zone'
      Origin = 'Zone'
      Size = 50
    end
  end
  object dsSuburb: TDataSource
    DataSet = qrySuburb
    Left = 464
    Top = 256
  end
  object VirtualImageList1: TVirtualImageList
    Images = <
      item
        CollectionIndex = 0
        CollectionName = 'backspace'
        Name = 'backspace'
      end>
    ImageCollection = ImageCollection1
    Width = 32
    Height = 32
    Left = 424
    Top = 416
  end
  object ImageCollection1: TImageCollection
    Images = <
      item
        Name = 'backspace'
        SourceImages = <
          item
            Image.Data = {
              }
          end>
      end>
    Left = 304
    Top = 416
  end
end
