...
'    If intResponse = vbYes Then
    ...
    If vReturnValue = 1 Then
        ...
        ' ------------------------------------------------------------------------------------------
        Set rst = CurrentDb.OpenRecordset("SELECT dbo_CoreGroup.CoreGroupID FROM dbo_CoreGroup", dbOpenSnapshot, dbFailOnError + dbSeeChanges)
...
Private Function ConstructTests(ByVal lngInspectionOrderID As Long, ByVal lngSiteID As Long, ByVal lngCoreGroupID As Long, Optional ByVal IgnoreMissingErr As Boolean = False) As Boolean
    ...
    SQL = _
           "SELECT dbo_Station.StationID, dbo_Station.StationNum, dbo_Station.Location, " & _
           "dbo_Station.IsManaged, dbo_Station.Note, dbo_Equipment.EquipmentID, dbo_EquipType.ServiceInterval " & _
           "FROM (dbo_Station " & _
           "INNER JOIN dbo_Equipment ON dbo_Station.EquipmentID = dbo_Equipment.EquipmentID) INNER JOIN dbo_EquipType ON dbo_Equipment.EquipTypeID = dbo_EquipType.EquipTypeID " & _
           "WHERE ( " & _
    ...
    ' -----------------------------------------------
    Set rs2 = CurrentDb.OpenRecordset(SQL, dbOpenSnapshot, dbFailOnError + dbSeeChanges)
    If rs2.RecordCount > 0 Then
        ...
        If Not IsNull(rst) Then
            Do While Not rs2.EOF
                If Nz(rs2.Fields("ServiceInterval"), 0) = 0 Then
                    ' UNEXPECTED ERROR - NO SERVICE LEVEL
                    If Not IgnoreMissingErr Then
                        MsgBox "Unable to the find Service Interval for Equipment ID: " & CStr(rs2.Fields("EquipmentID")) & ".", _
                        vbOKOnly Or vbDefaultButton1, _
                        mdlCompany.GetCompanyCode() & " - Build Check List."
                    End If
                Else
                    ' COMPARE SERVICEINTERVAL Station_Equipment ... InspectionOrder
                    If rs2.Fields("ServiceInterval") = dblServiceInterval Then
                        With rst
                            ' ##############################################################
                            ...
                            ' IMPORTANT - ACCESS BOOLEAN BUG WITH MSQLEXPRESS BIT
                            ' if any of the booleans are unitialised - ie.NULL - the record is locked
                            If IsNull(rs2.Fields("IsManaged")) Then
                                .Fields("IsManaged") = 0
                            Else
                                .Fields("IsManaged") = rs2.Fields("IsManaged")
                            End If
                            
                            .Fields("EquipmentID") = rs2.Fields("EquipmentID")
                            .Fields("Passed") = False
                            .Update
