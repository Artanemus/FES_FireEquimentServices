...
Private Function GetNextMajorLifeCycle_A(aStationID As Long, Optional GetMinor As Boolean = False) As Variant
Dim v As Variant
Dim SQL As String
    Dim rst As DAO.Recordset
    ...
    On Error GoTo PROC_ERR
        GetNextMajorLifeCycle_A = Empty

    ' FINDS the LIFE CYCLE datetime for the equip
    SQL = _
    "SELECT dbo_Station.StationID, dbo_Station.LifeCycleSeed, dbo_EquipType.MajorInterval, dbo_EquipType.MinorInterval " & _
            ...
            If rst.RecordCount > 0 Then
                ...
                Case True
                    If Nz(rst![MinorInterval], 0) > 0 Then
                        GetNextMajorLifeCycle_A = DateAdd("m", rst![MinorInterval], LifeCycleSeedDT)
                    End If
                Case Else
                    If Nz(rst![MajorInterval], 0) > 0 Then
                        GetNextMajorLifeCycle_A = DateAdd("m", rst![MajorInterval], LifeCycleSeedDT)
                    End If
                End Select
            End If
    End If
...
PROC_ERR:
    ...
    If err.Number <> 0 Then
        msg = ModuleName & " GetNextMajorLifeCycle_A" & vbCrLf & _
...
Private Function GetNextMajorLifeCycle_B(aStationID As Long, Optional GetMinor As Boolean = False) As Variant
    Dim v As Variant
    Dim SQL As String
    Dim rst As DAO.Recordset
    Dim dbs As DAO.Database
    ...
    If aStationID > 0 Then
        ...
        If rst.RecordCount > 0 Then
            ...
            Case True
                If Nz(rst![MinorInterval], 0) > 0 Then
                    GetNextMajorLifeCycle_B = DateAdd("m", rst![MinorInterval], LifeCycleSeedDT)
                End If
            Case Else
                If Nz(rst![MajorInterval], 0) > 0 Then
                    GetNextMajorLifeCycle_B = DateAdd("m", rst![MajorInterval], LifeCycleSeedDT)
                End If
            End Select
        End If
    End If
...
PROC_ERR:
    ' display the system error
    If err.Number <> 0 Then
        msg = ModuleName & " GetNextMajorLifeCycle_B" & vbCrLf & _
              "Error # " & CStr(err.Number) & " was generated by " & err.Source & vbCrLf & err.Description
        MsgBox msg, , "Error", err.HelpFile, err.HelpContext
    End If
    Resume PROC_EXIT
...
' find the next lifecycle datetime for the equipment type located in the station
    ...
    If aStationID > 0 Then
        v = GetNextMajorLifeCycle_B(aStationID, GetMinor)
        If v = Empty Then
            ' use the STATIONS's seed-date to calculate the next LC DT
            v = GetNextMajorLifeCycle_A(aStationID, GetMinor)
        End If
        If v <> Empty Then
            If IsDate(v) Then
...
Public Function IsHydrantMajorMinor(aInspectionOrderID As Long, ByRef IsHydrantMajor_Err As Boolean, Optional doMinor As Boolean = False) As Boolean
    ...
    'Set rst = dbs.OpenRecordset("xqryLifeCycle_Hydrant", dbOpenDynaset, dbFailOnError + dbSeeChanges)
    'rst.Filter = "[InspectionOrderID] = " & CStr(aInspectionOrderID)
    'rst.Requery
    
    SQL = "SELECT * FROM dbo_EquipType WHERE EquipTypeID = 1"
    ...
    Else
        ...
        Do While Not rst.EOF
            If rst![UseGlobalLifeCycle] = False Then
            ...
            Else
                ...
                Else
                    ...
                    End If
                    ' check all params ..
                    If Nz(rst![RequestedDT], 0) > 0 And Nz(rst![LifeCycleSeed], 0) > 0 And Nz(MInterval, 0) > 0 Then
                        If IsDate(rst![RequestedDT]) And IsDate(rst![LifeCycleSeed]) Then
                            ' date difference in months - divided by the majorInterval
                            v = DateDiff("m", rst![LifeCycleSeed], rst![RequestedDT]) / MInterval
                            ' if this is less than the service interval - it needs lifecycle service
                            If v <= mdlCompany.GetGlobalDefServiceInterval() Then
                                ...
                                ' **************************************************
                                If Nz(rst![LevelNum], 0) > 0 Then
                                    If rst![LevelNum] = 2 Then
